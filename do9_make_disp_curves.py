...
# note that the only real differences between this and the non-tfpws version are the
# filenames (COR prefixes, _s suffixes, trifile identifiers)
####

curves = []

# find all station pairs with stacks
corfiles = np.array(glob('COR/stacks/tf_*_full_hdr.sacn_2_DISP.1'))  # full crosscorr only
...
for p in pairs:
    ...
    disp2file = 'COR/stacks/tf_%s_full_hdr.sacn_2_DISP.1' % p
    ...
    snrfile = 'COR/stacks/tf_%s_full_hdr.sacn_snr.cv.p.txt' % p
    trifiles = glob('COR/stacks/tf_%s_????.??.????.??.????.??_hdr.sacn_2_DISP.1' % p)
