...
corfiles = np.array(glob('COR/stacks/tf_*_full.sacn_2_DISP.1'))  # full crosscorr only
...
for p in pairs:
    ...
    disp2file = 'COR/stacks/tf_%s_full.sacn_2_DISP.1' % p
    ...
    snrfile = 'COR/stacks/tf_%s_full.sacn_snr.cv.p.txt' % p
    trifiles = glob('COR/stacks/tf_%s_????.??.????.??.????.??.SAC_2_DISP.1' % p)
